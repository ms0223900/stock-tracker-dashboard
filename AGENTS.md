...
- **Vercel Cron**（選用）：`vercel.json` 設定對 `GET /api/cron/check-prices` 定時觸發；須設定 `CRON_SECRET`，與請求 `Authorization: Bearer` 一致（見 [`docs/spec.md`](docs/spec.md) 第十節）。前端輪詢仍呼叫無密鑰之 `/api/check-prices`。
