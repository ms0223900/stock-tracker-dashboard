...
| **Step 2** | 請 AI 加 log 同時印出「API 原始資料」和「程式算出來的值」，不用我們人工再算一次。 | `console.log`             |
| **Step 3** | 並排比對，發現 high / low / open / volume 全部算錯         | 肉眼比對或是 AI 輔助比對 Console 輸出 |
| **Step 4** | 確認 root cause：不同欄位需要不同的聚合方式                     | 請 AI 二次驗證其中演算邏輯           |


**Debug 節奏：** 從「明顯覺得圖表怪怪的」→「看到 raw data vs computed 的落差」→「發現是股價資料聚合邏輯有誤」→「確認 root cause」。
