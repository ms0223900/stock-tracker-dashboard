...
**以便** 使用者未開網頁時仍能定期評估是否發送 LINE（及／或 Telegram，依 spec 第九節並存策略）

**輸入格式**：
- Vercel 專案：`vercel.json`（或平台後台）設定 `crons`，`path` 指向 `/api/check-prices`（或專用 cron Route，但須呼叫同一套 `refreshWatchlistPrices`）；排程表達式由產品決定（參考 Notion 範例 `*/5 * * * *`，並注明方案 quota）
...
**輸出格式**：
- Cron 使用 **GET** 呼叫時仍能執行完整檢查（與功能 spec 第三節 3.2 一致）；現況 Route **僅 POST**，須改為 **GET 與 POST 共用同一 handler**（皆呼叫 `refreshWatchlistPrices`）；**GET** 須驗證 `CRON_SECRET`（或同等機制），**POST** 維持前端輪詢現有行為（不強制 secret，除非日後另開需求）
